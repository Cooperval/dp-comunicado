...
type TipoContaPagar = {
    ID_TIPOCONTASPAGAR: number;
    COD_TIPOCONTASPAGAR: number;
    DESCRICAO: string;
    SITUACAO: 'A' | 'I' | null;
};

type TiposContasPagarTabProps = {
    token: string;
    urlApi: string;
};
...
export default function TiposContasPagarTab({ token, urlApi }: TiposContasPagarTabProps) {
    ...
    const [sortConfig, setSortConfig] = useState<{
        key: 'COD_TIPOCONTASPAGAR' | 'DESCRICAO' | 'SITUACAO';
        direction: 'asc' | 'desc';
    } | null>(null);
    ...
    const fetchTipos = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${urlApi}/fluxo-caixa/tipos-contas-pagar`, {
    ...
    const filteredAndSorted = useMemo(() => {
        let items = [...tipos]; // <-- sempre crie uma cópia nova

        // Filtro
        if (search) {
            const termo = search.toLowerCase();
            items = items.filter(tipo =>
                tipo.COD_TIPOCONTASPAGAR.toString().includes(termo) ||
        ...
        if (sortConfig) {
            items.sort((a, b) => {
                ...
                if (sortConfig.key === 'COD_TIPOCONTASPAGAR') {
                    aVal = Number(aVal);
                    bVal = Number(bVal);
                }
        ...
        return items;
    }, [tipos, search, sortConfig]);

    const handleSort = (key: 'COD_TIPOCONTASPAGAR' | 'DESCRICAO' | 'SITUACAO') => {
        setSortConfig(prev => {
    ...
    const openDialog = (mode: 'create' | 'edit', data?: TipoContaPagar) => {
        if (mode === 'create') {
            setForm({ cod_tipocontaspagar: '', descricao: '', situacao: 'A' });
        } else if (data) {
            setForm({
                cod_tipocontaspagar: String(data.COD_TIPOCONTASPAGAR),
    ...
    const handleSave = async () => {
        const codigo = form.cod_tipocontaspagar.trim();
        ...
        setSaving(true);
        try {
            const isEdit = dialogMode === 'edit' && dialogData?.ID_TIPOCONTASPAGAR;
            const url = isEdit
                ? `${urlApi}/fluxo-caixa/alterar-tipos-contas-pagar/${dialogData!.ID_TIPOCONTASPAGAR}`
                : `${urlApi}/fluxo-caixa/adicionar-tipos-contas-pagar`;
            ...
            const res = await fetch(url, {
                ...
                body: JSON.stringify({
                    cod_tipocontaspagar: Number(codigo),
            ...
            if (!res.ok) {
                ...
                throw new Error(err.error || "Erro ao salvar tipo de conta");
            }

            toast({
                title: "Sucesso!",
                description: isEdit ? "Tipo atualizado com sucesso" : "Tipo criado com sucesso",
            });
        ...
        <>
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div className="space-y-2">
                            ...
                            <CardDescription>Gerencie os tipos de contas utilizados no sistema</CardDescription>
                            <CardDescription>
                                <strong>Importante:</strong> qualquer alteração nos Tipos de Contas
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" onClick={fetchTipos} disabled={loading}>
                ...
                <CardContent>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className="cursor-pointer" onClick={() => handleSort('COD_TIPOCONTASPAGAR')}>
                                    <div className="flex items-center gap-2">Código {getSortIcon('COD_TIPOCONTASPAGAR')}</div>
                                </TableHead>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                filteredAndSorted
                                    ...
                                    .map(tipo => (
                                        <TableRow key={tipo.ID_TIPOCONTASPAGAR || tipo.COD_TIPOCONTASPAGAR}>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>
                            {dialogMode === 'create' ? 'Novo Tipo de Conta' : 'Editar Tipo de Conta'}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                type="number"
                                placeholder="Ex: 101"
                                value={form.cod_tipocontaspagar}
                                onChange={(e) => setForm(prev => ({ ...prev, cod_tipocontaspagar: e.target.value }))}
