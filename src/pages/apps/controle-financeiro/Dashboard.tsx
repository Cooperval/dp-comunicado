...
import MetricCard from '@/components/dashboard/controle-financeiro/MetricCard';
import FinancialChart from '@/components/dashboard/controle-financeiro/FinancialChart';
...
const Dashboard = () => {
  ...
  const getCurrentMonthData = () => {
    ...
    // Margem Líquida (%)
    const netMargin = latest.receitaTotal > 0 
      ? (latest.lucroLiquido / latest.receitaTotal) * 100 
      : 0;
    
    const previousNetMargin = previous.receitaTotal > 0 
      ? (previous.lucroLiquido / previous.receitaTotal) * 100 
      : 0;
    ...
    const averageTicket = latest.transactionCount > 0 
      ? latest.receitaTotal / latest.transactionCount 
      : 0;
    
    return {
      revenue: latest.receitaTotal,
      netProfit: latest.lucroLiquido,
      netMargin,
      averageTicket,
      revenueGrowth,
      profitGrowth,
