...
export function useMovimentacao() {
  ...
  const fetchData = useCallback(async (params = {}) => {
    const { dataInicio, dataFim, tipo, tiposConta, grupoEmpresa } = params || {};
    ...
    try {
      ...
      if (tiposConta && tiposConta !== "all") {
        qs.append("tiposConta", tiposConta); // valores esperados: "1", "3", "4"
      }
      if (grupoEmpresa) qs.append("grupoEmpresa", grupoEmpresa);
      const url = `${urlApi}/fluxo-caixa/movimentacoes` + (qs.toString() ? "?" + qs.toString() : "");
