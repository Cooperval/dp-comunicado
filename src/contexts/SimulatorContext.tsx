...
import {
  calcularPrecosLiquidos,
  calcularDRE,
  calcularDREPorProduto,
  calcularCpvPorProduto,
...
export const SimulatorProvider: React.FC<SimulatorProviderProps> = ({ children }) => {
    ...
    try {
      ...
      const newScenario: SavedScenario = {
        ...
        data: {
          ...
          precoLiquidoEtanolHidratadoCana:
            (data.salesPrices.hydratedEthanolGross || 0) * (1 - (data.salesPrices.icmsEthanol || 0) / 100) -
            (data.salesPrices.pisCofinsEthanol || 0),
          precoLiquidoEtanolHidratadoMilho:
            (data.salesPrices.hydratedEthanolGross || 0) * (1 - (data.salesPrices.icmsEthanol || 0) / 100) -
            (data.salesPrices.pisCofinsEthanol || 0),
          precoLiquidoEtanolAnidroCana:
            (data.salesPrices.anhydrousEthanolGross || 0) - (data.salesPrices.pisCofinsEthanol || 0),
          precoLiquidoEtanolAnidroMilho:
            (data.salesPrices.anhydrousEthanolGross || 0) - (data.salesPrices.pisCofinsEthanol || 0),
          ...
          custoCanaUnitarioTotal:
            (data.productionCosts.caneRawMaterial || 0) +
          ...
          custoMilhoUnitarioTotal:
            (data.productionCosts.cornRawMaterial || 0) +
          ...
          // Receitas por Matéria-Prima
          receitaCanaTotal:
            (dreCalculations.receitaAcucarCana || 0) +
          ...
          impostosCana: (() => {
            ...
            const icmsEthanolValue =
              (data.salesPrices.hydratedEthanolGross || 0) * ((data.salesPrices.icmsEthanol || 0) / 100);
            const hydratedEthanolNet =
              (data.salesPrices.hydratedEthanolGross || 0) -
              (data.salesPrices.pisCofinsEthanol || 0) -
              icmsEthanolValue;
            const anhydrousEthanolNet =
              (data.salesPrices.anhydrousEthanolGross || 0) - (data.salesPrices.pisCofinsEthanol || 0);
            ...
            return (
              impostoUnitHidratado * (dreCalculations.prodEHC || 0) + impostoUnitAnidro * (dreCalculations.prodEAC || 0)
            );
          })(),
          impostosMilho: (() => {
            // Calcula preços líquidos etanol (impostos apenas para etanóis)
            const icmsEthanolValue =
              (data.salesPrices.hydratedEthanolGross || 0) * ((data.salesPrices.icmsEthanol || 0) / 100);
            const hydratedEthanolNet =
              (data.salesPrices.hydratedEthanolGross || 0) -
            ...
            // Imposto Milho = (Imposto Unit. Hidratado × Prod. EHM) + (Imposto Unit. Anidro × Prod. EAM)
            return (
              impostoUnitHidratado * (data.corn.hydratedEthanol || 0) +
          ...
          receitaLiquidaCana: (() => {
            ...
            const impostoUnitAnidro = (data.salesPrices.anhydrousEthanolGross || 0) - anhydrousEthanolNet;
            const impostosCana =
              impostoUnitHidratado * (dreCalculations.prodEHC || 0) +
          ...
          receitaLiquidaMilho: (() => {
            ...
            // Calcula impostos do milho (apenas etanóis)
            const icmsEthanolValue =
              (data.salesPrices.hydratedEthanolGross || 0) * ((data.salesPrices.icmsEthanol || 0) / 100);
            const hydratedEthanolNet =
              (data.salesPrices.hydratedEthanolGross || 0) -
            ...
            const impostoUnitHidratado = (data.salesPrices.hydratedEthanolGross || 0) - hydratedEthanolNet;
            const impostoUnitAnidro = (data.salesPrices.anhydrousEthanolGross || 0) - anhydrousEthanolNet;
            const impostosMilho = impostoUnitAnidro * (dreCalculations.prodEAM || 0);

            return receitaMilho - impostosMilho;
          })(),
          receitaLiquidaOutras:
            (dreCalculations.receitaCO2Cana || 0) +
            (dreCalculations.receitaCO2Milho || 0) +
        ...
        prev.map((scenario) => ({
          ...
          data: {
            ...
            impostosCana: (() => {
              ...
              // Calcula preços líquidos para etanol
              const icmsEthanolValue =
                (scenarioData.salesPrices.hydratedEthanolGross || 0) *
                ((scenarioData.salesPrices.icmsEthanol || 0) / 100);
              const hydratedEthanolNet =
                (scenarioData.salesPrices.hydratedEthanolGross || 0) -
                (scenarioData.salesPrices.pisCofinsEthanol || 0) -
                icmsEthanolValue;
              const anhydrousEthanolNet =
                (scenarioData.salesPrices.anhydrousEthanolGross || 0) -
                (scenarioData.salesPrices.pisCofinsEthanol || 0);
            ...
            impostosMilho: (() => {
              ...
              // Imposto Milho = (Imposto Unit. Hidratado × Prod. EHM) + (Imposto Unit. Anidro × Prod. EAM)
              return (
                impostoUnitHidratado * (scenarioData.corn.hydratedEthanol || 0) +
                impostoUnitAnidro * (dreCalculations.prodEAM || 0)
              );
            })(),
            ...
            margemMilho: (() => {
              ...
              const receitaLiquidaMilho = receitaMilho - impostosMilho;
              const cpvMilho =
                (cpvCalculado.hydratedEthanolCornCpv || 0) * (scenarioData.corn.hydratedEthanol || 0) +
                (cpvCalculado.anhydrousEthanolCornCpv || 0) * (dreCalculations.prodEAM || 0) +
                (cpvCalculado.ddgCpv || 0) * (dreCalculations.prodDDG || 0) +
                (cpvCalculado.wdgCpv || 0) * (dreCalculations.prodWDG || 0);
              return receitaLiquidaMilho - cpvMilho;
            ...
            return {
              ...
              data: {
                ...
                receitaLiquidaCana: (() => {
                  ...
                  const totalReceita = receitaCana + receitaMilho;
                  const impostosCana =
                    totalReceita > 0 ? (dreCalculations.totalImpostos || 0) * (receitaCana / totalReceita) : 0;
                  return receitaCana - impostosCana;
                })(),
                receitaLiquidaMilho: (() => {
                  const receitaCana =
                  ...
                  const totalReceita = receitaCana + receitaMilho;
                  const impostosMilho =
                    totalReceita > 0 ? (dreCalculations.totalImpostos || 0) * (receitaMilho / totalReceita) : 0;
                  return receitaMilho - impostosMilho;
                })(),
                receitaLiquidaOutras:
                  (dreCalculations.receitaCO2Cana || 0) +
  ...
  const loadScenarioForEditing = (id: string): boolean => {
    ...
    if (!scenario.originalData) {
      console.error(
        "Este cenário foi salvo em uma versão anterior e não possui dados editáveis. Delete-o e salve novamente.",
      );
      return false;
    }
  ...
  const importMultipleScenarios = (
    scenariosData: SavedScenario[],
