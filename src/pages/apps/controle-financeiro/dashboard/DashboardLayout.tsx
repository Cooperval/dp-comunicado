...
export const NAV_GERENCIAL: NavItem[] = [
  { name: "Visão Geral", icon: BarChart3, href: "/apps/controle-financeiro/dashboard", current: true },
  { name: "Indicadores", icon: PieChart, href: "/apps/controle-financeiro/indicators" },
  //{ name: "Saldos Bancários", icon: Wallet, href: "/apps/controle-financeiro/bank-balances" },
  // { name: 'Cenários', icon: Target, href: '/scenarios' },
  { name: "Análise de Margens", icon: TrendingUp, href: "/apps/controle-financeiro/margins" },
  { name: "Fluxo por Natureza", icon: FileText, href: "/apps/controle-financeiro/financial-statement" },
  { name: "Orçamento", icon: DollarSign, href: "/apps/controle-financeiro/budget" },
  { name: "Fluxo de Caixa", icon: Calendar, href: "/apps/controle-financeiro/cash-flow" },
  { name: "Consultar OFX", icon: ArrowLeftRight, href: "/apps/controle-financeiro/transactions" },
  { name: "Consultar XML", icon: FileText, href: "/apps/controle-financeiro/nfe-list" },
];

export const NAV_OPERACIONAL: NavItem[] = [
  { name: "Carregar OFX", icon: Upload, href: "/apps/controle-financeiro/upload-ofx" },
  { name: "Carregar XML", icon: FileX, href: "/apps/controle-financeiro/upload-nfe" },
  { name: "Classificar OFX", icon: Tags, href: "/apps/controle-financeiro/transaction-classification" },
  // { name: 'Equipe', icon: Users, href: '/team' },
  // { name: 'Orçamento', icon: DollarSign, href: '/budget' },
];

export const NAV_ORGANIZACAO: NavItem[] = [
  { name: "Estrutura", icon: Building, href: "/apps/controle-financeiro/organization" },
];


export const NAV_ADMIN: NavItem[] = [
  // { name: "Configurações", icon: Settings, href: "/settings" },
  { name: "Gerenciar Hierarquia", icon: TreePine, href: "/apps/controle-financeiro/hierarchy-management" },
];

// util: encontra o título com base na URL atual
export function getPageTitle(pathname: string): string {
  const match =
    [...NAV_GERENCIAL, ...NAV_OPERACIONAL, ...NAV_ORGANIZACAO, ...NAV_ADMIN].find(
      (item) => pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href)),
    ) || NAV_GERENCIAL.find((i) => i.href === "/");

  return match?.name ?? "Controle Financeiro";
}

const AppSidebar = () => {
  const { user, profile, company, signOut, subscribed } = useAuth();
  ...
  const getTrialDaysRemaining = () => {
    ...
    return Math.ceil((new Date(company.trial_ends_at).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
  };
  ...
  const handleSignOut = async () => {
    try {
      ...
      navigate("/apps/controle-financeiro/auth");
    } catch (error) {
    ...
    <Sidebar className="border-sidebar-border">
      <SidebarHeader className="border-b border-sidebar-border p-4">
        <div className="flex items-center gap-3">
          ...
          {open && (
            <div>
              <h1 className="text-lg font-bold text-sidebar-foreground">Controle Financeiro</h1>
      ...
      <SidebarFooter className="border-t border-sidebar-border p-4">
        <div className="space-y-2">
          {subscribed && open && (
            <Badge variant="outline" className="justify-center w-full mb-2 gap-2 bg-success/10 border-success text-success">
              <Clock className="h-3 w-3" />
              Plano Pro
            </Badge>
          )}
          {!subscribed && company?.subscription_plan === "trial" && trialDays !== null && trialDays >= 0 && open && (
...
const DashboardLayout = () => {
  const { user, profile, company, loading, subscribed } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  ...
  const isTrialUser = company?.subscription_plan === "trial" && isSubscriptionActive && !subscribed;

  // Verifica se deve mostrar a tela de trial expirado
  const shouldShowTrialExpired = () => {
    // Nunca bloquear usuários do plano Free
    if (company?.subscription_plan === 'free') {
      return false;
    }

    // Se assinatura está ativa, não mostrar
    ...
    <>
      ...
      <SidebarInset className="bg-gradient-to-br from-background via-secondary/20 to-primary/5">
        ...
        <main className="flex-1 p-8">
          <>
            {isTrialUser && company.trial_ends_at && (
              <TrialBanner trialEndsAt={company.trial_ends_at} onUpgradeClick={handleUpgradeClick} />
            )}
            <Outlet />
          </>
        </main>
